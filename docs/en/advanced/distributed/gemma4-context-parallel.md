...
Select `flash_attn: triton_gqa`, and set `ulysses_context_parallel_size` to a divisor of the distributed world size.
For an eight-GPU run, `ulysses_context_parallel_size: 4` creates two data-parallel groups with four context-parallel
ranks each. See `examples/train_lora/gemma4_lora_sft_cp.yaml` for a complete configuration.
...
`per_device_train_batch_size * gradient_accumulation_steps * (world_size / ulysses_context_parallel_size)`. Trainer
and DeepSpeed may still display the physical world-size batch, which includes the repeated CP copies; use the unique
batch size when choosing the learning rate or interpreting sample throughput.

The query-head count must be divisible by `ulysses_context_parallel_size`. KV heads are sharded when divisible; when a
Gemma 4 global-attention layer has fewer KV heads than CP ranks, the attention package expands KV heads before its
all-to-all.
